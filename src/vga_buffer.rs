...
pub struct Writer {
    ...
    color_code: ColorCode,
    buffer: &'static mut Buffer, // ‘static means that the reference is valid for the whole program run time
}
...
use spin::Mutex; // Instead of blocking, the threads simply try to lock it again and again in a tight loop, thus burning CPU time until the mutex is free again.
use lazy_static::lazy_static; // Instead of computing its value at compile time, the static lazily initializes itself when accessed for the first time.
...
pub fn _print(args: fmt::Arguments) {
    use core::fmt::Write;
    WRITER.lock().write_fmt(args).unwrap(); // RAII
}
